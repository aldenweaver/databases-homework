...
public class CatalogTest extends SimpleDbTestBase {
    ...
    @Test public void updateTable() {
    	...
    	SkeletonFile file = new SkeletonFile(-4, Utility.getTupleDesc(2));
    	
    	// add table with id but no name
    	Database.getCatalog().addTable(file);
    	
    	// add table with same id, this time with name (should update or override previous entry)
    	Database.getCatalog().addTable(file,"NewName");
    	assertEquals("NewName",Database.getCatalog().getTableName(-4));
    }
    ...
    @Test public void clear() {
    	Database.getCatalog().addTable(new SkeletonFile(-5, Utility.getTupleDesc(1)));
    	Database.getCatalog().clear();
    	try {
    		Database.getCatalog().getTupleDesc(-5);
